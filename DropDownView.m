...
- (id)initWithArrayData:(NSMutableArray*)data cellHeight:(CGFloat)cHeight heightTableView:(CGFloat)tHeightTableView paddingTop:(CGFloat)tPaddingTop paddingLeft:(CGFloat)tPaddingLeft paddingRight:(CGFloat)tPaddingRight refView:(UIView*)rView animation:(AnimationType)tAnimation openAnimationDuration:(CGFloat)openDuration closeAnimationDuration:(CGFloat)closeDuration{
	...
	if ((self = [super init])) {
		...
		self.view.frame = CGRectMake(refFrame.origin.x-paddingLeft,refFrame.origin.y-30,refFrame.size.width+paddingRight, heightTableView);
...
-(void)callTable{
    CGRect refFrame = refView.frame;
	uiTableView = [[UITableView alloc] initWithFrame:CGRectMake(0,self.paddingTop,refFrame.size.width+paddingRight, (animationType == BOTH || animationType == BLENDIN)?heightTableView:1) style:UITableViewStylePlain];
